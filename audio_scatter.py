...
def play(filename, player_id, autoplay=True, normalize=True):
    update_display(Audio(filename=filename, autoplay=autoplay, normalize=normalize), display_id=player_id)


def audio_scatter(data, x='x', y='y', audio_path='audio_path', text='text', player_id=None,normalize=True, circle_radius=0.03, figsize=None, **kwargs):
    ...
    def onclick(event):
        ...
        fig.canvas.draw()
        play(r[audio_path], player_id,normalize=normalize)
